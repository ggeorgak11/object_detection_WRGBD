				 ...
				 )
{
    ...
    for(ff=0;ff<nb_feature;ff++){ //for every feature on the test image
        for(cc=0;cc<nb_code;cc++){ //for every codebook entry
            Chi2Dis   = 0.0;
            weight_sum_row  = 0;
            for(bb=0;bb<nb_bin;bb++){ //for every bin in the descriptor
			...
			if(weight_sum_row<sc_threshold[0])
                ...
                for(bb=0;bb<nb_bin;bb++){                                    
                    double_dummy1   = codebook[bb*nb_code+cc]; //one bin code entry value
