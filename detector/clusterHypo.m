function [hypo_list,score_list, idx_list] = clusterHypo( hypo_list, score_list,scale_ratio_list, params)
...
params.elps_ab = [(params.obj_height/params.obj_aspect_ratio)*0.15, params.obj_height*0.15];
...
for iter = 1:params.nb_iter    
    ...
    elps_dist= hypo_dx.*hypo_dx/(params.elps_ab(1)*params.elps_ab(1))+ hypo_dy.*hypo_dy/(params.elps_ab(2)*params.elps_ab(2));
    if(do_scale)
    ...
    % keep hypo with the max score in the neighborhood. position of the
    % hypo does not change
